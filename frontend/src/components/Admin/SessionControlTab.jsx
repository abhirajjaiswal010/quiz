          ...
          <div className="space-y-6">
            <div className="flex gap-3">
              <div className="flex-1">
                <label className="label uppercase tracking-widest text-[10px] text-white/50 font-bold mb-1.5">Unique Quiz ID</label>
                <input
                  type="text"
                  value={quizId}
              ...
              <div className="w-24">
                <label className="label uppercase tracking-widest text-[10px] text-white/50 font-bold mb-1.5">Mins</label>
                <input
                  type="number"
                  value={duration}
              ...
              <div className="flex-col flex items-center justify-center mb-3">
                <label className="text-[9px] text-white/40 uppercase font-bold mb-3 tracking-tight border-b border-white/5 pb-1">Allow Tab</label>
                <button
                  onClick={() => setAllowTabSwitching(!allowTabSwitching)}
                  className={`w-12 h-6 rounded-full transition-all duration-300 relative ${allowTabSwitching ? 'bg-emerald-500 ' : 'bg-white/20'}`}
                >
            ...
            <div className="pt-4 grid grid-cols-1 gap-4">
              <button onClick={handleCreate} disabled={loading} className="btn-primary w-full  text-white font-monserrat uppercase py-4  active:scale-[0.98] transition-all rounded-xl ">Create Session</button>
          ...
          <div className="space-y-6">
            ...
            {sessionInfo && (
              <div className="p-6 rounded-2xl bg-white/5 border border-white/10 space-y-4">
                <p className="text-[11px] text-white/40 uppercase font-bold tracking-[0.2em] border-b border-white/5 pb-2.5">Live Session Telemetry</p>
                ...
                {status && sessionInfo.quizDetails?.startedAt && (
                  <div className="flex justify-between items-center bg-[#4fb3ff]/10 p-4 rounded-xl border border-[#4fb3ff]/20">
                    <span className="text-[#4fb3ff] text-xs font-bold uppercase tracking-widest">Global Clock</span>
                    <AdminTimer
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div>
                    <span className="text-white/40 text-[10px] uppercase font-bold block mb-1">Total Pool Questions</span>
                    <span className="text-white font-bold text-lg">{sessionInfo.totalQuestions}</span>
                  </div>
                  <div>
                    <span className="text-white/40 text-[10px] uppercase font-bold block mb-1">Time Constrains</span>
                    <span className="text-white font-bold text-lg">{sessionInfo.quizDetails.duration}m</span>
                  </div>
                </div>
                ...
                <div className="pt-2">
                  <div className={`flex items-center justify-between p-3 rounded-xl border ${allowTabSwitching ? 'bg-amber-500/10 border-amber-500/20' : 'bg-emerald-500/10 border-emerald-500/20'}`}>
                    ...
                    <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${allowTabSwitching ? 'text-amber-500' : 'text-emerald-400'}`}>
